...
tm_shape(df_roads) + tm_lines("y_hat_rf")
...
df_roads$y_hat_RF <- crashesrfmap$predictions$values
df_roads_drop <- df_roads |> st_drop_geometry()
df_roads$y_hat_rf <- predict(crashesRF, data=crashgeom_spdf)
crashesRF$ranger.arguments
```
...
```{r}
vip.5_Bag1 <- DALEX::model_parts(explainer = explainer_Bag1, 
...
```{r}
library(caret)
inTraining <- createDataPartition(cr_hyp$combined_score, p = .75, list = FALSE)
training <- cr_hyp[ inTraining,]
testing  <- cr_hyp[-inTraining,]
...
set.seed(825)
gbmFit2 <- caret::train(form, data = training, 
                 method = "gbm", 
